...
一个 Python + PySide6 桌面端与原生 Android 工具，用于批量通知搜索平台发现自有网站的 URL 和 Sitemap，让新发布的官网、博客、产品页和内容页更快进入搜索引擎发现流程。
...
官方站长提交接口都要求某种形式的网站控制权证明，但验证方式不同：

| 类型 | 控制权证明 |
|---|---|
| Google、百度、Bing、Yandex | 在对应站长平台添加并验证站点，凭据只能操作账号有权管理的站点 |
| IndexNow | 在目标网站部署与请求一致的 Key 文件，证明可以控制该网站 |
| 360、神马 | 使用站长平台时同样按平台要求添加、验证或管理站点 |

搜索引擎不需要网站所有者主动提交，也可以自然抓取互联网上的公开页面；但“公开可访问”不等于“任何人都能通过站长 API 提交”。小红书、携程、微信公众号等第三方链接可以由本工具自动提取、保存和查询状态，不能使用用户自己的凭据代替平台所有者提交，也不能保证或强制收录。
