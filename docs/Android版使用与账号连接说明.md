...
- 提交历史永久保存到本机 SQLite 数据库；
- 每条记录显示提交日期、网址、提交次数和搜索引擎状态徽标；
- 右上角刷新按钮重新查询历史网址的收录状态。
...
需要注意：能提取链接不代表用户有权向站长平台提交该链接。Google、百度、Bing 和 Yandex 的站长接口要求当前账号有权管理目标站点；IndexNow 要求能在目标网站部署 Key 文件。360 和神马的站长入口也面向站点所有者或管理者。

小红书、携程、微信公众号等第三方页面可能被搜索引擎自然发现，但用户不能使用自己的站长凭据代替平台所有者提交，也不能通过本工具强制收录。Android 版仍可提取这类分享文案中的链接，并保存到输入框或历史记录中；直接提交时，官方接口会拒绝不属于当前站点的 URL。
...
每次执行提交后，Android 版会按 URL 保存一条历史记录。重复提交相同 URL 时不会无限创建重复行，而是更新最近提交时间并增加提交次数。
...
右上角 `⟳` 用于刷新历史状态。当前 Android 版可用 Bing Webmaster API 做精确查询；Google 移动端查询需要发行方完成 OAuth 应用注册，百度、360 和神马没有稳定公开的精确 URL 查询 API，因此不会通过抓取搜索网页伪造结果。

历史数据库位于 Android 应用私有目录。正常关闭、重启和手机重启不会丢失；卸载 App、清除应用数据或恢复出厂设置会删除本地历史。
